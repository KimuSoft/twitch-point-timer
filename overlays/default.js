...
function TimerItem({data}) {
  ...
  return <Mui.Paper sx={{borderRadius: '16px', width: '100%', overflow: 'hidden'}}>
    <Mui.Box sx={{padding:2}}>
      <div style={{fontSize: 24, fontWeight: 600}}>{data.name}</div>
      <div style={{fontSize: 18, fontWeight: 400}}>{data.remainingTime}</div>
    </Mui.Box>
...
function Timer() {
  ...
  return <div style={{width: '100%', display: 'flex', gap: 16, flexDirection: 'column', justifyContent: 'center', alignItems: 'center'}}>
    {timers.map((x, i) => <TimerItem key={i} data={x}/>)}
  </div>
}
